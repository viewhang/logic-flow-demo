<template>
  <a-tabs tab-position="left">
    <a-tab-pane label="添加动作">
      <div v-for="item in nodeList" :key="item.type">
        <a-button class="add-node-btn" type="primary" size="mini" @click="$_addNode(item)">{{ item.label }}</a-button>
      </div>
    </a-tab-pane>
    <a-tab-pane label="添加组">
      <a-button class="add-node-btn" type="primary" size="mini" @click="$_addTempalte">模板</a-button>
    </a-tab-pane>
  </a-tabs>
</template>
<script>
